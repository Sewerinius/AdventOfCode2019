module B where
    ...
    bfs :: Set.Set (Int, Int) -> Seq ((Int, Int), ((Data.Map.Map Int Int, Int, Int), [Int], [Int])) -> ((Int, Int), (Data.Map.Map Int Int, Int, Int))
    bfs visited queue = let 
        (pos, comp) :< rest = viewl queue
        (state, _, out:_) = run (comp, False)
        (newVisited, newData) = nextNeighbours visited pos 
        newComps = map (\(pos, dir) -> (pos, (state, [dir], []))) newData
        newQueue = rest >< fromList newComps
        trac = trace ("bfsTrac -> pos=" ++ show pos ++ ", out=" ++ show out) 0
        -- in case trac + out of 
        in case out of 
            2 -> (pos, state)
    ...
    bfs2 :: Set.Set (Int, Int) -> Seq ((Int, Int), Int, ((Data.Map.Map Int Int, Int, Int), [Int], [Int])) -> Int -> Int
    bfs2 visited queue v = let 
        left = viewl queue
        (pos, val, comp) :< rest = left
        (state, _, out:_) = run (comp, False)
        (newVisited, newData) = nextNeighbours visited pos 
        ...
        in case left of 
            EmptyL -> v
            _ -> if out == 0 then bfs2 visited rest v else bfs2 newVisited newQueue val
    ...
    prog x = do
        ...
        let queue = fromList $ map (\(pos, dir) -> (pos, ((sa, 0, 0), [dir], []))) newData
        let (pos, state) = bfs newVisited queue

        let visited2 = Set.singleton pos
        let (newVisited2, newData2) = nextNeighbours visited pos 
        let queue2 = fromList $ map (\(pos, dir) -> (pos, 1, (state, [dir], []))) newData
        let out = bfs2 newVisited2 queue2 0
        print out
