module B where
    ...
    getOreValue m s = let
        ...
        in loop (singleton s 6226152) ts -- found this with a binary search while looking how the ore value changes
                                         -- TODO: Actually try to do this problem in reverse
    
    main = do
        v <- fromFile "in.txt"
    
        -- print $ count ((==2) . snd) $ cnv contents
        let val = getOreValue (toMap v) "FUEL"
        print val
